...
def _worker_compute_processor(compute_queue, result_queue, stop_event, processor_name, processor_args=None):
...
class MultiStagePipeline:
    ...
    def start(self, items: List[Any]) -> bool:
        ...
        if not ProcessorRegistry.get(self.compute_processor_name):
            raise ValueError(f"Compute processor '{self.compute_processor_name}' not found in registry")
        ...
        for i in range(self.config.compute_workers):
            process = mp.Process(
                ...
                args=(self.compute_queue, self.result_queue, self.stop_event, 
                     self.compute_processor_name, self.compute_processor_args),
    ...
    def _process_io_batch(self):
        ...
        batch_count = 0
        while (self.io_index < len(self.io_items) and 
