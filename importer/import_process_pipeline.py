...
class IOCPUPipeline:
    ...
    def _io_worker(self):
        ...
        while True:
            try:
                ...
                try:
                    # Call I/O function with or without argument
                    if task_arg is not None:
                        io_result = self.io_function(task, task_arg)
                    else:
    ...
    def start(self):
        ...
        logger.info(
            f"Starting pipeline with {self.io_workers} I/O workers and {self.cpu_workers} CPU workers"
        )
    ...
    def process_tasks(
        self, tasks: List[Any], task_args: Optional[List[Any]] = None
    ) -> List[Any]:
        """
        ...
        Args:
            ...
            task_args: Optional list of arguments for each task (same length as tasks)
        ...
        # Validate arguments
        if task_args is not None and len(task_args) != len(tasks):
            raise ValueError("task_args must have the same length as tasks")
        ...
        try:
            ...
            for i, task in enumerate(tasks):
                task_arg = task_args[i] if task_args else None
                self.task_queue.put((task, task_arg))
            ...
            while completed < total_tasks:
                try:
                    ...
                    logger.warning(
                        "Timeout waiting for results - some tasks may have failed"
                    )
                    break
